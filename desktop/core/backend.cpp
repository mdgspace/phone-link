    ...
    : QObject(parent)
{
    ...
    m_router.systemHandler()->setLocalDevice(m_deviceId, m_deviceName);
...
void Backend::onFileReceived(const QString &transferId, const QString &fileName, qint64 totalBytes, const QString &filePath)
{
...
void Backend::onPairingRequested(const QString &deviceId, const QString &pin)
{
    if (!m_activeClient)
    {
    ...
    // The PIN was generated by the phone (PairingService.generatePin) and
    // sent to us in pairing_request{pin}. We display it for the desktop
    // user to confirm and echo it back verbatim - we never generate our
    // own, since the phone compares the echoed PIN against what it sent.
    m_pairingPin = pin;
